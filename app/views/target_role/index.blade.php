...
{{ Form::open(array('url' => 'target_role/batchDelete')) }}
        ...
        <table id="example" class="table table-striped  table-bordered"  >
            ...
            <tbody>
              ...
              @foreach($targetRole as $target_role)
                <tr>

                   <td><input type="checkbox" name="id[]"  id="checkbox" class="myCheckbox2" value="{{ $target_role->id }}"></td>
                   <td>{{ $target_role->code }}</td>
                   <td>{{ $target_role->title }}</td>
                   <td>{{ $target_role->description }}</td>

                   <td>
                      <a href="{{ URL::route('target_role.edit', ['id'=>$target_role->id])  }}" class="btn btn-default" data-toggle="modal" data-target="#edit-modal" data-toggle="tooltip" data-placement="left" title="Edit" href="#"><span class="glyphicon glyphicon-edit text-info"></span></a>

                      <a data-href="{{ URL::route('target_role.destroy',['id'=>$target_role->id]) }}" class="btn btn-danger" data-toggle="modal" data-target="#confirm-delete" data-toggle="tooltip" data-placement="left" title="Delete" href="" ><span class="glyphicon glyphicon-trash text-danger"></span></a>

                      <a href="{{ URL::route('target_role.show', ['id'=>$target_role->id])  }}" class="btn btn-default" data-toggle="modal" data-target="#showModal" data-toggle="tooltip" data-placement="left" title="Show/View" href=""><span class="glyphicon glyphicon-list-alt text-info"></span></a>
