...
<div style="padding:3%; ">
   {{ Form::open(array('route'=>['amw.assign-examiner-comments',$question_data->id],'method' => 'POST')) }}
        ...
        {{Form::hidden('commented_to', $question_data->e_faculty_user_id)}}
        {{Form::hidden('commented_by', Auth::user()->get()->id )}}
            ...
            <div class="form-group">
                  <div class="col-lg-6" style="padding-left: 0;">
                      <div>{{Form::label('commented_to', 'Evaluator')}}</div>
                      <div>{{ Form::select('commented_to', $examiner_faculty_lists, $question_data->e_faculty_user_id, ['class' => 'form-control']) }}</div>
                  </div>
                  <div class="col-lg-6">
                      <div>{{Form::label('commented_to', 'Status')}}</div>
                      <div>{{ Form::select('e_status', ['requested'=> 'Requested', 'cancel'=>'Cancel','assigned' => 'Assigned'], $question_data->e_staus,['class' => 'form-control']) }}</div>
                  </div>
            </div>
            <p>&nbsp;</p>
