    ...
    <div class="box box-solid ">
        ...
        <div class="col-sm-12">
           ...
           <div class="pull-right col-sm-4" style="padding-top: 1%;">
                <a href="{{ URL::route('manage-rnc-ar-bill') }}" type="button" class="pull-right btn btn-sm btn-info" > >> RNC Invoice</a>
           </div>
        </div>
            ...
            <tbody>
                @foreach($data as $values)
                 <tr style="{{$values->status=='invoiced' ? 'background-color: burlywood' : '' }}">
                    <td><b>
                        {{ link_to_route($values->status!="invoiced" ?'rnc-bill-details' : 'rnc-bill-details',$values->relRncResearchPaper->title,['trn_id'=>$values->id], ['data-toggle'=>"modal", 'data-target'=>"#modal-pc"]) }}
                    </b></td>
