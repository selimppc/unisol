    ...
    <div class="box box-solid ">
        <div class="col-sm-12">
           <div class="pull-left col-sm-4"> <h3> {{$pageTitle}} </h3>  </div>
            <a href="{{ URL::route('applicant-receivable-index') }}" type="button" class="pull-right btn btn-xs btn-default" > << Back to Applicant Billing</a>
        </div>
        ...
        <table id="example" class="table table-striped  table-bordered" >
            ...
            <tbody>
                @foreach($data as $values)
                 <tr>
                    <td><b>
                    {{ link_to_route('applicant-ar-voucher', $values->associated_id, ['associated_id'=>$values->associated_id, 'coa_id'=>$values->acc_chart_of_accounts_id], ['data-toggle'=>"modal", 'data-target'=>"#modal-pc"]) }}
                    </b></td>
                    <td>{{ $values->name }}</td>
